...
//
// `magnitude`'s alt is deliberately a runaway paragraph, well past
// VISUAL_ALT_MAX_CHARS: it is the only offline exercise of clampText() in
// server/debate.js, so a Mock+Mock debate proves the guard trims at a word
// boundary and ends in an ellipsis instead of chopping a word in half. Its
// caption reading "…" on the projector is the test passing, not a bug. That
// cap is a runaway guard set far above the length the director prompt asks
// for, so this entry has to stay conspicuously over it to keep firing — and
// every other entry has to stay well under, since a caption that merely runs
// long is meant to display in full.
const DIRECTOR_TURNS = [
  ...
  { archetype: 'magnitude', prompt: 'A tall bar for the reported gap between methods and a short bar for the run-to-run spread, with the noise floor crossing both.', alt: 'Two bars set side by side, the reported effect at its full published height and the run-to-run spread beside it, with the noise floor drawn straight across both so the gap can be sized by eye, and a note under the axis restating that the difference between the two bars is smaller than the spread within either bar on its own, which is the whole argument of this turn.' },
