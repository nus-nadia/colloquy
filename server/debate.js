...
//
// The caps in prompts/visuals.js are runaway guards rather than the length the
// model was asked for — see VISUAL_ALT_MAX_CHARS — so this should fire on
// almost no real turns, and a caption that merely runs long displays in full.
// When it does fire, the result has to read as a deliberate trim: a bare
// slice() chopped the caption mid-word, which from the back of a lecture hall
...
// struck-through row.
//
// `trimmed` names any field clampText() had to shorten, for the same reason.
// An over-length reply is otherwise invisible apart from an ellipsis on the
// projector, so the caller logs it — and because the caps sit far above the
// length the prompt asks for, a guard firing on most turns means the prompt
// and the cap have drifted apart, not that the model misbehaved.
