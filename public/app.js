// Colloquy frontend — vanilla JS, no build step, no frameworks. Loaded as an
// ES module (index.html), which is what lets it share render.js with the server.

// The markdown renderer lives in render.js because the server imports the
// same module to build the HTML transcript export — see CLAUDE.md. It is the
// ONLY code path allowed to set .innerHTML from model-generated text.
import { escapeHtml, renderStatementHTML } from './render.js';
...
// Download is a two-option menu: .md is the plain transcript, .html is the
// self-contained page that carries the generated visuals with it (the .md
// export can only describe them — a relative image URL dies with the file).
function setDownloadMenuOpen(open) {
  downloadMenu.hidden = !open;
  downloadBtn.setAttribute('aria-expanded', String(open));
  downloadBtn.classList.toggle('is-open', open);
}

downloadBtn.addEventListener('click', (e) => {
  e.stopPropagation(); // the document listener below would close it right back
...
downloadMenu.addEventListener('click', (e) => {
  ...
  if (!state.debateId) return;
  const format = item.dataset.format === 'html' ? 'html' : 'md';
  const a = document.createElement('a');
  a.href = `/api/debates/${state.debateId}/transcript?format=${format}`;
  a.download = `colloquy-${state.debateId}.${format}`;
...
document.addEventListener('keydown', (e) => {
  // Runs before the shortcut handler's Escape branch, which would otherwise
  // drop out of presentation mode while the menu was the thing on top.
